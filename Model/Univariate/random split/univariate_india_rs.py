...
Original file is located at
    https://colab.research.google.com/drive/1D9Ls1L3qt9rWWnIMJoT609uWC2ZUhy5S
"""
...
def MODEL_LSTM(name,x_train,x_test,y_train,y_test,Num_Exp,n_steps_in,n_steps_out,Epochs,Hidden):
    ...
    # model.save('/content/drive/My Drive/covid project/Model/Results/Delhi/' + name + '_lstm.h5')
...
def MODEL_Bi_LSTM(name, x_train,x_test,y_train,y_test,Num_Exp,n_steps_in,n_steps_out,Epochs,Hidden):
    ...
    model.compile(optimizer='adam', loss='mse')
    model.summary()
    # model.save('/content/drive/My Drive/covid project/Model/Results/Delhi/' + name + '_bi-lstm.h5')
...
def MODEL_EN_DC(name, x_train,x_test,y_train,y_test,Num_Exp,n_steps_in,n_steps_out,Epochs,Hidden):
    ...
    model.compile(optimizer='adam', loss='mse')
    model.summary()
    # model.save('/content/drive/My Drive/covid project/Model/Results/Delhi/' + name + '_en-dc.h5')
    Best_RMSE=10000000000   #Assigning a large number 
    future_prediction = np.zeros([Num_Exp,60])
    start_time=time.time()
    for run in range(Num_Exp):
...
def Plot(name,labels,width,LSTM,Bi_LSTM,EN_DC,yer1,yer2,yer3,xlabel,ylabel,Gname,cap):
    ...
    ax.legend()
    fig.tight_layout()
    plt.savefig("/content/drive/My Drive/covid project/Model/Results/ALL_INDIA/"+name+"_"+Gname+".png",dpi=300)
    plt.show()
...
for i in range(1,2):
    problem=i
    if problem ==1:
        TrainData = pd.read_csv('/content/drive/My Drive/covid project/Data/Processed_Data/random_shuffle/train_Totals.csv',index_col = 0)
        TrainData = TrainData.values
        TestData = pd.read_csv('/content/drive/My Drive/covid project/Data/Processed_Data/random_shuffle/test_Totals.csv',index_col = 0)
